...
vol(){
    val=$(sh ~/bin/bar/BSDNixVolume.sh -get)
    if [ $val -gt 50 ] ; then 
        echo "\\uf028%{T3} $val%%{T-}"
    elif [ $val -gt 0 ] ; then
        echo "\\uf027%{T3} $val%%{T-}"
...
while true ; do {
    ...
    song="%{F$color2}%{T4}%{c}$(bash ~/bin/get-song.sh 42 '')%{F-}%{T-}"
    ...
    printf " %b \\n" \
        "%{T4}$ws1$ws2$ws3$ws4$ws5$ws6$ws7$ws8 %{-u}%{T-}$layout $song %{r}$bat $wifi $vpn $vol $dat" 
